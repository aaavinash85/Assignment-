...
#include<string>
#include<string.h>
#include<vector>      //By writing #include <vector>, you are telling the compiler to not only use your own code, but to also compile a file called vector. This file is actually somewhere on your hard drive 
using namespace std;
class mp3file{
    private:
    string f_name;                       
...
class directory{
    ...
    bool isMp3(){
        if(ent->d_type==8){
            ...
            if(temp==".mp3")                 // checks for mp3 file if found then it returns true otherwise false
                return true;
            else
                return false;
        }
